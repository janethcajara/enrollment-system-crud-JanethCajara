...
if ($stud_id <= 0 || $first_name === '' || $middle_initial === '' || $last_name === '' || $program_id <= 0) {
...
try {
    $stmt = $pdo->prepare("INSERT INTO student_tbl (stud_id, first_name, middle_initial, last_name, program_id, allowance) VALUES (?, ?, ?, ?, ?, ?)");
    $stmt->execute([$stud_id, $first_name, $middle_initial, $last_name, $program_id, $allowance]);
